...
- offline `pnpm test`: 2 files passed, 1 database file skipped without an explicit test database; 6 tests passed and 4 database tests skipped;
- `pnpm test:db`: 1 file and 4 PostgreSQL integration tests passed;
- `pnpm build`: passed.
...
- BFF cookie issuance, rotation, logout, CSRF/origin protection, recovery, and session cleanup;
