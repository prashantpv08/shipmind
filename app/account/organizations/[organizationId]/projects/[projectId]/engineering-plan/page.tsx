...
export default async function EngineeringPlanPage({ params }: { params: Promise<{ organizationId: string; projectId: string }> }) {
  ...
  return <main className="account-shell">
    ...
    <section className="account-card engineering-plan-card" aria-labelledby="engineering-plan-heading">
      ...
      {state.status === 'ready' ? <div className="account-state"><div className="project-access-heading"><div><h2>{state.project.name}</h2><p>{state.project.id} · graph v{state.project.graphVersion} · {state.project.status.replaceAll('_', ' ')}</p></div><nav aria-label="Project navigation"><Link className="account-text-link" href={projectHref}>Sources</Link><Link className="account-text-link" href={`${projectHref}/backlog`}>Backlog</Link><Link className="account-text-link" href={projectsHref}>Projects</Link></nav></div><EngineeringPlanWorkflow organizationId={organizationId} projectId={projectId} graphVersion={state.project.graphVersion} canGenerate={state.canGenerate} initialPlan={state.plan} /></div> : null}
    </section>
  </main>;
}
