...
export default async function ProjectSourcePage({ params }: { params: Promise<{ organizationId: string; projectId: string }> }) {
  ...
  return <main className="account-shell">
    ...
    <section className="account-card project-source-card" aria-labelledby="project-source-heading">
      ...
      {state.status === 'ready' ? <div className="account-state"><div className="project-access-heading"><div><h2>{state.project.name}</h2><p>{state.project.id} · graph v{state.project.graphVersion} · {state.project.status.replaceAll('_', ' ')}</p></div><nav aria-label="Project navigation"><Link className="account-text-link" href={projectsHref}>Projects</Link><Link className="account-text-link" href={`${projectsHref}/${encodeURIComponent(projectId)}/business-context`}>Business Context</Link><Link className="account-text-link" href={`${projectsHref}/${encodeURIComponent(projectId)}/engineering-plan`}>Engineering Plan</Link><Link className="account-text-link" href={`${projectsHref}/${encodeURIComponent(projectId)}/backlog`}>Backlog</Link></nav></div><SourceWorkflow organizationId={organizationId} projectId={projectId} sources={state.sources} initialRun={state.latestRun} canManage={state.canManage} /></div> : null}
    </section>
  </main>;
}
