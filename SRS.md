...
**Supersedes:** Axiom Commercial SRS 4.3 dated 2026-08-11
...
| DR-018 | Commercial code follows KISS and YAGNI, applies DRY after the Rule of Three or an already-proven shared invariant, keeps ownership cohesive with dependencies pointing inward, and turns critical boundaries into mechanical checks. Platform Zod contracts are the single source for deterministic OpenAPI and the generated web client. | Do not introduce speculative abstractions, handwritten web copies of platform payloads, cross-context concrete-repository coupling, mutable CI action references, or undocumented contract drift. Exceptions require current caller evidence, the narrowest safe boundary, regression coverage, and an SRS decision when they alter product or architecture scope. |

## 18. Current implementation ledger

This ledger records the honest repository state at SRS 4.4. A capability is not commercially complete merely because a deterministic fixture or prototype screen exists.

| Capability | State | Current evidence and boundary |
|---|---|---|
| Contract, truth model, and architecture boundaries | Foundation complete | Commercial principles, canonical graph, truth statuses, provider boundaries, framework-neutral application errors, centralized ETag and idempotency behavior, bounded schema ownership, and local-only development rules are established. All 43 authoritative HTTP operations are emitted from runtime Zod contracts into deterministic OpenAPI and a generated web client; CI checks contract and architecture drift. |
| PostgreSQL foundation and migrations | Substantial foundation | The main local database is migrated through `0020`; the complete 81-test PostgreSQL suite verifies forward behavior, tenant and idempotency boundaries, and ordered rollback through every paired down migration on a disposable database. The current platform `main` revision also reproduced the migration and PostgreSQL integration gates in GitHub Actions. |
...
| Business Context | Partial | Deterministic outcomes, actors, workflows, success measures, applicability, exact versions, reviews, audit, ETags, idempotency, visible web review, and fail-closed downstream enforcement exist. An authorized reviewer can resolve an exact `NEEDS_DECISION` applicability preview through a consequential-action preview that creates a new canonical graph version and immutable organization-scoped `HUMAN_CONFIRMED` decision with rationale, optimistic concurrency, retry-safe idempotency, audit, and downstream invalidation. The organization project list now provides a bounded aided-migration queue: it composes paginated project metadata with validated exact preview and current-baseline responses, classifies analysis, decision, generation, regeneration, review, revision, complete, archived, and unavailable states, preserves partial failures, exposes the exact project review path, and provides no bulk decision or approval. A self-contained disposable-database browser matrix proves the queue plus explicit applicable, explicit not-applicable, unresolved, stale, rejected, anonymous, viewer, cross-tenant, retry, successful mutation, audit, and reload-persistence behavior through the real browser, BFF, platform, and PostgreSQL boundaries. Current heuristic classification is not semantically qualified for commercial use; existing projects still require explicit human review, and jobs, policies, constraints, risks, business-model entities, exact source-span inspection, and Agent Kernel extraction remain open. |
...
| Web and BFF migration boundary | Controlled migration | Commercial browser routes use a common thin-BFF boundary and a generated platform SDK. Root traffic enters the commercial account flow; historical prototype pages are isolated under `/prototype`; undeclared legacy APIs fail closed by default and can be enabled only for non-production loopback migration work. Four Blob-backed historical adapters remain isolated migration inputs pending an explicit data-retention decision. |
...
| Security, privacy, testing, and operability | Not launch-ready | Local CI definitions pin action commits, enforce repository hygiene and dependency thresholds, and run contract, architecture, lint, type, test, migration, and build gates. GitHub Actions run `31512103293` passed both platform jobs from current `main` revision `f35d59dd01831e5e640b6afbc2f793a1a487925f`, reproducing the static, contract, provider, evaluation, build, migration, repository, and API-integration gates from a clean checkout. Protected-branch and required-check enforcement remain unverified, as do equivalent current remote runs for the web and infrastructure repositories. ASVS/API mapping, complete UI/functional/API/database/security test obligations, separately authorized penetration testing, performance evidence, retention/deletion jobs, SLOs, runbooks, backup restore, WCAG review, and incident evidence remain open. |
...
On 2026-08-12, web lint, typecheck, architecture and generated-contract drift checks, the production build, and 163 tests passed; five tests remained skipped by their configured environment gates. The dedicated project-list regression suite passed three cases covering mixed migration classification, visible per-project partial failure, and fail-closed nested authorization. The self-contained applicability runner created the exact disposable PostgreSQL database, applied all platform migrations, started isolated platform and web processes, passed six browser scenarios including the organization migration queue and the complete current applicability decision matrix, and removed the database afterward. A point-in-time query of the current local inventory found 24 current projects still migration-required: 21 exact previews are `NEEDS_DECISION`, three are source-grounded `APPLICABLE`, and none has a current Business Context version or review; these are pending human decisions and reviews, not completed work. The current platform `main` revision `f35d59dd01831e5e640b6afbc2f793a1a487925f` has reviewable GitHub history, and [GitHub Actions run 31512103293](https://github.com/prashantpv08/axiom-platform/actions/runs/31512103293) completed successfully on 2026-08-11 with both the static/contract/provider/evaluation/build job and the PostgreSQL repository/API-integration job green. Dependency-audit and infrastructure evidence remain the 2026-08-10 snapshot: web and platform production dependencies had no known vulnerability at the moderate threshold; their complete development graphs retained one and two moderate advisories respectively and no high advisory; infrastructure policy, Terraform 1.15.8 formatting, and validation passed without backend initialization or cloud access. Protected-branch enforcement and equivalent current remote web and infrastructure evidence remain open.

### 18.2 Active product and delivery flags

| Severity | Flag | Required resolution |
|---|---|---|
| GREEN | The current platform `main` revision has reviewable remote history and GitHub Actions clean-checkout run `31512103293` passed both the static/build and PostgreSQL integration jobs. | Keep the exact platform checks required and preserve the successful run as point-in-time evidence; do not generalize it to later revisions. |
| AMBER | Protected-branch and required-check enforcement have not been verified or configured across all three repositories, and equivalent current remote web and infrastructure runs have not been recorded. | After an exact external-change preview and authorization, configure required checks and branch protection, then record current clean-checkout evidence for the remaining repositories. |
...
| AMBER | The aided Business Context migration queue is implemented and tested, but all 24 current local projects remain migration-required: 21 need an explicit applicability decision and three need exact generation and review. Their historical artifacts stay inspectable and cannot receive silent new downstream approvals. | Authorized humans must follow each exact review path and approve or reject its current evidence; never bulk-decide or auto-approve applicability or business meaning. |
| GREEN | The organization migration queue plus exact applicability decision UI and platform mutation are covered by a self-contained six-scenario browser matrix using an exact disposable database. It proves queue classification and exact review links, explicit applicable, explicit not-applicable, needs-decision, exact preview and cancel, keyboard selection, both successful decisions, stale and invalid/already-explicit rejection, anonymous, viewer and cross-tenant denial, unchanged-key retry, immutable audit creation, and persistence after reload. | Keep `pnpm test:e2e:applicability` in the local and future clean-checkout gates; extend it with the compatible approved Experience Baseline journey only after the P2 domain exists, without claiming that currently missing capability. |
...
- [x] The platform repository has reviewable version-control history and CI can reproduce its lint, typecheck, test, migration, and build evidence from a clean checkout.
...
- [x] Browser E2E proves applicable, not-applicable, needs-decision, stale, rejected, unauthorized, cross-tenant, retry, and success paths without fabricated progress or evidence.
...
| Version | Date | Change |
|---|---|---|
| 4.4 | 2026-08-12 | Added the governed organization-level P0 Business Context migration queue with validated exact preview/current-baseline composition, explicit decision/generation/regeneration/review/revision/complete/archived/unavailable states, partial-failure visibility, role-aware exact review links, and no bulk approval path. Added focused platform-state tests and extended the six-scenario disposable-database browser matrix to cover the queue. Recorded successful platform clean-checkout GitHub Actions evidence for current `main` and completed that P0 CI criterion. The measured local inventory still contains 24 migration-required projects—21 needing a human applicability decision and three needing exact generation and review—so the final P0 human-review criterion remains intentionally open. |
| 4.3 | 2026-08-11 | Added a self-contained applicability browser-matrix runner that creates the exact disposable `axiom_test_applicability_e2e` database, applies current platform migrations, starts isolated local platform and Next.js processes, and removes the database afterward. Six real browser scenarios now prove explicit applicable/not-applicable/unresolved states, exact preview and cancel, keyboard selection, both successful human decisions, stale and invalid/already-explicit rejection, anonymous/viewer/cross-tenant denial, unchanged-idempotency-key retry, immutable audit creation, and reload persistence. Marked only the current P0 browser-matrix item complete; existing-project review, remote clean-checkout CI, semantic Business Context extraction, and the commercial Experience Baseline remain open. |
| 4.2 | 2026-08-11 | Added the first governed migration path for unresolved experience applicability: an authorized reviewer receives an exact consequential-action preview, records `APPLICABLE` or `NOT_APPLICABLE` with rationale against the exact current preview and ETag, and creates a new canonical graph version plus immutable organization-scoped decision, audit, idempotency result, and downstream invalidation. Added migration `0020`, the 43rd generated HTTP operation, the thin BFF and accessible retry-safe web interaction, tenant/role/stale/replay/conflict tests, full forward/reverse migration evidence, and updated local verification counts. Existing-project review and the complete browser decision matrix remain open. |
| 4.1 | 2026-08-10 | Made KISS, evidence-triggered YAGNI, Rule-of-Three DRY, cohesive ownership, inward dependencies, contract single-source-of-truth, fail-closed boundaries, and mechanical CI enforcement explicit. Isolated the historical web prototype and legacy APIs, migrated commercial BFF calls to generated platform operations, established deterministic Zod-to-OpenAPI-to-client drift checks, centralized platform application errors, ETags, and idempotency, split schema and billing persistence by bounded ownership, added complete migration rollback verification, pinned CI actions and toolchains, hardened dependency and repository policies, and validated Terraform locally without initializing a backend or creating cloud resources. No remote CI, branch protection, deployment, or external side effect was performed. |
