...
test.skip(
  ...
  'Legacy migration E2E requires AXIOM_LEGACY_PROTOTYPE_ENABLED=true on loopback.',
);
...
test('Axiom guides a project from landing through documents, optional wireflow, approved architecture, and delivery planning', async ({ page }) => {
  ...
  const templateGallery = page.getByLabel('Wireframe template gallery');
  await expect(templateGallery.locator('article')).toHaveCount(12);
  await templateGallery.getByRole('button', { name: /AI copilot/ }).click();
  ...
  await expect(page.getByRole('heading', { name: 'Architecture diagrams' })).toBeVisible();
  const architectureInputs = page.getByRole('region', { name: 'Choose the stack and where it will run.' });
  await architectureInputs.getByRole('button', { name: 'API/service', exact: true }).click();
  await architectureInputs.getByRole('button', { name: 'Confirm inputs & recalculate' }).click();
  await expect(architectureInputs.getByRole('status').filter({ hasText: 'Inputs saved.' })).toBeVisible({ timeout: 20_000 });
  ...
  const approveArchitecture = page.getByRole('button', { name: /Approve architecture/ });
  await expect(approveArchitecture).toBeEnabled();
