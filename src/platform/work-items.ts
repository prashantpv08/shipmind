...
export type WorkItemReviewState =
  | { status: 'ready'; project: z.infer<typeof PlatformProjectSchema>; readiness: PlatformProjectReadiness | null; preview: PlatformWorkItemGenerationPreview | null; canGenerate: boolean; canReview: boolean }
...
export async function getWorkItemReview(organizationIdInput: string, projectIdInput: string): Promise<WorkItemReviewState> {
  ...
  const [organizationResponse, projectResponse, readinessResponse, previewResponse] = await Promise.all([
  ...
  const required = [organizationResponse, projectResponse, readinessResponse];
  ...
  if (!organization.success || !parsedProject.success || !parsedReadiness.success || parsedReadiness.data.projectId !== parsedProject.data.id || parsedReadiness.data.graphVersion !== parsedProject.data.graphVersion || (preview !== null && !preview.success)) return { status: 'unavailable', message: 'The platform returned an unexpected backlog response.' };
  return {
    status: 'ready',
