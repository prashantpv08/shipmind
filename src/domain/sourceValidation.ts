import { AnalysisRunSchema, SourceSpanSchema, type AnalysisRun, type SourceDocument } from './schemas';
...
export function validateSourceSpan(document: SourceDocument, span: unknown) {
  ...
  if (actual !== parsed.text) {
    throw new Error(`Invalid source span ${parsed.id}: text does not match original brief`);
  }
  return parsed;
}
...
export function validateAnalysisRun(run: unknown, document: SourceDocument): AnalysisRun {
  ...
  const check = (id: string, item: { status: string; sourceSpanIds: string[] }) => {
    if (item.status === 'GROUNDED' && item.sourceSpanIds.length === 0) {
      throw new Error(`${id} is grounded without source spans`);
    }
    for (const spanId of item.sourceSpanIds) {
      if (!spans.has(spanId)) throw new Error(`${id} references unknown source span ${spanId}`);
    }
  };
