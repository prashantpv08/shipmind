...
describe('Day 1 fixture analysis', () => {
  ...
  it('calculates readiness deterministically', () => {
    const run = buildFixtureAnalysis(notifyFlowSourceDocument);
    const first = calculateReadiness(run.projectId, run.id, run);
    const second = calculateReadiness(run.projectId, run.id, run);
    expect(first).toEqual(second);
    expect(first.overall).toBe(58);
  });
  ...
  it('preserves valid state after failed analysis attempt', () => {
    ...
    let state = valid;
    try {
      buildFixtureAnalysis({ ...notifyFlowSourceDocument, spans: [{ ...notifyFlowSourceDocument.spans[0], text: 'bad' }] });
    } catch {
      state = valid;
    }
