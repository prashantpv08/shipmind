...
export function calculateReadiness(projectId: string, runId: string, partial: Pick<AnalysisRun, 'requirements' | 'nonFunctionalRequirements' | 'gaps'>): ReadinessScore {
  const blockers = partial.gaps.filter((g) => g.severity === 'BLOCKING').length;
  const nfr = (category: string) => partial.nonFunctionalRequirements.some((r) => r.category === category && r.status === 'GROUNDED');
  const categories = [
    ...
    { id: 'failure', label: 'Failure handling', score: partial.gaps.some((g) => g.title.includes('retry')) ? 35 : 65, rationale: 'Retry is required, but policy and outage behavior are missing.' },
