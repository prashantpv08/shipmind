...
export function buildProjectIntelligence(input: {
  ...
  return {
    ...
    techStack: buildTechStack(input.projectId, input.entities),
  };
}
...
function buildTechStack(projectId: string, entities: KnowledgeEntity[]): TechStackRecommendation[] {
  ...
  const recommendations: Array<Omit<TechStackRecommendation, 'id' | 'sourceEntityIds' | 'truthStatus'>> = [
    { layer: 'EXPERIENCE', recommendation: 'Responsive Next.js and TypeScript web application', rationale: 'Supports an accessible product surface, server-rendered project views, and focused interactive workspaces without introducing a separate frontend platform.', alternatives: ['React SPA', 'Native mobile client when mobile-only requirements are confirmed'] },
    { layer: 'APPLICATION', recommendation: 'Modular TypeScript application service', rationale: 'Keeps delivery and end-to-end verification simple while preserving explicit domain boundaries that can be extracted later.', alternatives: ['Go services for CPU-bound or independently owned domains', 'Managed functions for isolated bursty handlers'] },
    { layer: 'DATA', recommendation: 'Managed PostgreSQL as the transactional source of truth', rationale: 'Provides constraints, transactions, indexing, audit-friendly relations, and a clear ownership model for evolving product records.', alternatives: ['SQLite for a single-node prototype', 'Document database when aggregate and access patterns justify it'] },
    { layer: 'ASYNC', recommendation: 'Managed durable queue only for confirmed asynchronous workflows', rationale: 'Isolates slow or unreliable external work without requiring an event-streaming platform before throughput and ordering needs are proven.', alternatives: ['In-process jobs for bounded MVP work', 'Event streaming when measured ordering and replay requirements justify it'] },
