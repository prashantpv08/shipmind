...
8. Review the generated Jira epic and child stories. When Jira is connected, explicitly confirm that plan to create the hierarchy; then select a published story and prepare its controlled coding contract in Axiom Build Studio. The current MVP ends on that same governed handoff page; repository authorization, project-specific code generation, and fixed verification are explicitly labeled as the next implementation phase.
...
- The project journey ends on its own governed coding handoff page. It exposes the compiled task for IDE copy, labels repository execution and verification as the next phase, and does not redirect users into an unrelated demonstration project.
...
- Blocking document-review notices and `HIGH`/`BLOCKER` severity labels share the red danger treatment while retaining explicit text labels for accessible status communication.
- Saved graphs reconcile superseded `UNKNOWN` sentinels on open; document decisions include human-confirmed clarifications, and risk sections compile the structured gap register with explicit status instead of presenting a misleading empty state.
...
- Each wireflow pattern exposes a four-screen preview before generation. Preview titles, mapped source or human-confirmed context, open gaps, and coverage counts are deterministically compiled from the active project graph; the template only supplies the reusable journey shape.
- Architecture review now captures budget, hosting ownership, team skills, expected scale, web/mobile surface, and mobile capabilities before approval. Axiom compares lean, managed, and scale-ready packages—including HTML/PHP/SQLite and cross-platform mobile options—and records the human-selected stack in the ADR/HLD. Hosting floors are workload- and currency-specific, distinguish a possible static free tier from dynamic or managed applications, and exclude promotional pricing, taxes, domains, email, and app-store fees; runtime and actual cost remain unverified until measured.
